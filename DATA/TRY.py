...
val_data = Signal_Data(ds_type='val',starting_channel=0,ending_channel=0,
                             num_batches=400,device=device)
test_data = Signal_Data(ds_type='test',starting_channel=0,ending_channel=0,
                             num_batches=400,device=device)
...
# model = MLP_SD()
model.load_state_dict(torch.load('./exp_state/sigt_model.pth'))
...
print('Train Accuracy:', evaluation('train'))
print('Val Accuracy:', evaluation('val').item())
print('Test Accuracy:', evaluation('test').item())
